...
async function getListings() {
  try {
    ...
    const { data, error } = await supabase
      .from("listings")
      .select("id,title,status,price_amount,currency,condition,updated_at,metadata,listing_images(public_url)")
      .eq("seller_id", user.id)
      .is("deleted_at", null)
      .order("updated_at", { ascending: false });
    if (error) throw error;
    return { listings: (data ?? []) as ListingRow[], authRequired: false, error: null };
  } catch (error) {
    return { listings: [] as ListingRow[], authRequired: false, error: error instanceof Error ? error.message : "Unable to load listings." };
  }
}
...
export default async function ListingsDashboardPage() {
  ...
  return (
    <DashboardShell title="My listings" description="Manage drafts, active listings, AI-generated recommendations, and moderation status from one place.">
      ...
      {authRequired && (
        <Card><CardContent className="p-6">Sign in to create drafts, publish listings, and manage your seller inventory.</CardContent></Card>
      )}
      {error && (
      ...
      <div className="grid gap-4">
        {listings.map((listing) => {
          const moderationStatus = typeof listing.metadata?.moderation_status === "string" ? listing.metadata.moderation_status : "pending";
          const fraudScore = typeof (listing.metadata?.ai_listing as Record<string, unknown> | undefined)?.fraudRiskScore === "number" ? (listing.metadata?.ai_listing as { fraudRiskScore: number }).fraudRiskScore : null;
          return (
            <Card key={listing.id}>
              <CardHeader className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="mt-2 flex flex-wrap gap-2"><Badge>{listing.status}</Badge><Badge>moderation: {moderationStatus}</Badge>{fraudScore !== null && <Badge><ShieldAlert className="h-3 w-3" /> fraud {fraudScore}/100</Badge>}</div>
                  </div>
                </div>
                <div className="flex gap-2">
                  <Button asChild variant="outline"><Link href={`/dashboard/listings/${listing.id}/edit`}><Edit className="h-4 w-4" /> Edit</Link></Button>
                  <form action={`/api/listings/${listing.id}`} method="post"><Button variant="ghost" disabled><Trash2 className="h-4 w-4" /> Delete in editor</Button></form>
                </div>
              </CardHeader>
            </Card>
          );
