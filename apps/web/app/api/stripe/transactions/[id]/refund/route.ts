...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const refund = await getStripe().refunds.create({ payment_intent: payment.provider_payment_id, amount, reason: payload.data.reason, metadata: { transaction_id: transactionId.data } });
