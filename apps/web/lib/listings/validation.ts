...
const listingFormBaseSchema = z.object({
    ...
    priceAmount: z.coerce.number().min(0.01, "Price must be greater than 0.").max(999999.99),
    ...
    locationCountry: z.string().trim().toUpperCase().regex(/^[A-Z]{2}$/).default("US"),
    shipsTo: z.array(z.string().trim().min(2).max(40)).max(30).default([]),
    fulfillmentOptions: z.array(z.enum(FULFILLMENT_OPTIONS)).min(1, "Choose shipping, pickup, or local delivery."),
